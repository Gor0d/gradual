...
4. Prototipar o fluxo de onboarding + geração de checklist via Claude API antes de construir o marketplace completo — é o maior risco técnico do produto. Pré-requisito já feito: fundação de auth (`lib/supabase/*`, `proxy.ts`, `lib/auth/ensure-user.ts`) e seed de dados (`packages/db-schema/seed.ts`, rodar com `npm run db:seed`) com o `event_type` "colacao_de_grau", as 5 categorias de fornecedor (beca, anel, fotografia, buffet, local) e um `checklist_template` v1 com 11 itens/prazos padrão — a IA personaliza/reordena em cima desse template, não gera do zero.
